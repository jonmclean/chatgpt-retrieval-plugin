...
res = conn.cursor().execute(f"SELECT id, embedding FROM {document_table_name} ORDER BY id ASC")
rows = res.fetchall()
...
# ~/source/diskann/DiskANN/build/tests/build_memory_index --data_type float --dist_fn l2 --data_path ./diskann.bin --index_path_prefix diskann_index
